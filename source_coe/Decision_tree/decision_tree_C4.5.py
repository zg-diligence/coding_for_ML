...
class DecisionTree:
    ...
    def entropyAttr(self, data_set, col):
        """计算数据集关于指定特征的熵"""
        values = self.extractValues(data_set, col)
        results = {key: values.count(key) for key in set(values)}
        probs = [v / len(data_set) for v in results.values()]
        return sum([-prob * log2(prob) for prob in probs])
    ...
    def bestSplitAttr(self, data_set, values, used_attr):
        ...
        max_gain_ratio, split_col, split_sets = 0, None, None
        for col in range(self.attr_num):
            if col in used_attr: continue
            attr_entropy =  self.entropyAttr(data_set, col)
            if attr_entropy == 0: continue
            ...
            gain_ratio = (current_entropy - childs_entropy) /attr_entropy
            if gain_ratio > max_gain_ratio:
                max_gain_ratio, split_col, split_sets = gain_ratio, col, sets
        return max_gain_ratio, split_col, split_sets
    ...
    def buildTree(self, data_set = None, used_attr = None, threshold = 0):
        ...
        max_gain_ratio, split_col, split_sets = self.bestSplitAttr(data_set, values, used_attr)
